...
mean Λ*
drift /1k
β_i heterogeneous, fixed
0.4989
18.06
−0.022
β shrinks where connected
0.4993
17.90
−0.002
β grows where connected
0.5031
17.98
−0.030
...
The system is not stationary — still relaxing, still growing, not yet at its ceiling. Tendency is then real but temporary, with a lifetime set by how far the system is from equilibrium.
Step magnitudes are asymmetric — gains small and frequent, losses large and rare.
...
Growing 𝒞. Formalise a completion space whose capacity increases with time and test whether 𝒜⁺ > 1/2 becomes sustainable. This is the last live route to tendency and should be attempted before anything else.
Measure magnitude asymmetry in a real linking system. Any empirical network with formation and dissolution events will do. If E[Δ|up] < E[Δ|down], S2″ applies and the system is fragile — a useful diagnostic independent of ln1.
Then the v0.1 §9.2 program: construct 𝒞 for a restricted computational domain.
Interpretive layer: still leave it.
